...
export function MoveToBag({ onClick, productAddedInCart }) {
	return (
	  <button
		onClick={onClick}
		className={`mt-3 w-[93px] font-["Satoshi"] text-[13px] py-0.5 px-2 text-center flex-nowrap rounded-full border-solid border relative overflow-hidden my-0 ${
		  productAddedInCart
			...
			: "bg-transparent text-[#012F3F] border-[#0090c2]"
		}`}
	  >
		<span
		  className={`transition-all duration-300 ${
			productAddedInCart ? "opacity-0" : "opacity-100"
		  }`}
		>
		  Move to bag
		</span>
		<span
		  className={`transition-all duration-300 absolute top-0 left-0 w-full h-full flex items-center justify-center ${
			productAddedInCart ? "opacity-100" : "opacity-0"
		  }`}
		>
		  Added to bag
		</span>
